...
export default function Filter({ filter, setFilter }: { filter: string; setFilter: any }) {
  ...
  return (
    ...
    <div className="flex gap-5 mb-5 distort">
      ...
      <button onClick={() => { setFilter("texts") }} className={`${(filter == 'texts' || filter == 'texts, lectures, special-projects') && 'active'} group flex items-center gap-2 uppercase cursor-pointer`}>
        ...
        <div className="text-left leading-[1]">Texts</div>
      </button>
      <button onClick={() => { setFilter("lectures") }} className={`${(filter == 'lectures' || filter == 'texts, lectures, special-projects') && 'active'} group flex items-center gap-2 uppercase cursor-pointer`}>
        <div>
          <div className="w-[22px] aspect-square flex justify-center items-center border-[3px] border-black relative">
            <div className="absolute text-[16px] hidden group-[.active]:block">
              <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 119 108" className="w-[15px]">
        ...
        <div className="text-left leading-[1]">Lectures</div>
      </button>
      <button onClick={() => { setFilter("special-projects") }} className={`${(filter == 'special-projects' || filter == 'texts, lectures, special-projects') && 'active'} group flex items-center gap-2 uppercase cursor-pointer`}>
        <div>
          <div className="w-[22px] aspect-square flex justify-center items-center border-[3px] border-black relative">
            <div className="absolute text-[16px] hidden group-[.active]:block">
              <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 119 108" className="w-[15px]">
        ...
        <div className="text-left leading-[1]">Special Projects</div>
      </button>
    </div>
  );
}
